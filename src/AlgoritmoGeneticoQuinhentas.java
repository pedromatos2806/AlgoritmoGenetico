...
public class AlgoritmoGeneticoQuinhentas {
    ...
    static final ForkJoinPool FORK_JOIN_POOL = 
        new ForkJoinPool(Runtime.getRuntime().availableProcessors() * 2);
    
    // ========== ESTRUTURAS OTIMIZADAS ==========
    static final Map<String, Double> fitnessCache = 
        Collections.synchronizedMap(new LinkedHashMap<String, Double>() {
            @Override
    ...
    static List<CromossomoUltra> evoluirComOtimizacoesExtremas(List<CromossomoUltra> populacao) {
        ...
        List<CromossomoUltra> novos = FORK_JOIN_POOL.submit(() ->
            IntStream.range(ELITE_SIZE, POPULACAO)
                .parallel()
                .mapToObj(i -> {
